...
# 1. Instanciar el TfidfVectorizer
tfidf = TfidfVectorizer(stop_words='english')

# 2. Crear una nueva columna que combine la descripción y el género
df['content'] = df['description'] + ' ' + df['genre'].apply(lambda x: ' '.join(eval(x)))  # Convirtiendo lista de géneros en string

# 3. Vectorizar la columna 'content' (descripción + géneros)
X = tfidf.fit_transform(df['content'])

# 4. Crear el modelo de KNN
knn = NearestNeighbors(n_neighbors=5, metric='cosine')
knn.fit(X)

# 5. Buscar películas similares a 'The Shawshank Redemption' (por título)
movie_name = 'Interstellar'  # Cambia esto por cualquier película que desees buscar

# Obtener el índice de la película seleccionada
movie_index = df[df['title'] == movie_name].index[0]

# Buscar las 5 películas más cercanas usando KNN
distances, indices = knn.kneighbors(X[movie_index], n_neighbors=6)

# Mostrar la película seleccionada y las recomendaciones
print(f"Película seleccionada: {df['title'][movie_index]}")
print("\nPelículas recomendadas:")

for idx in indices[0][1:]:
    print(f"- {df['title'][idx]} (Rating: {df['rating'][idx]})")
